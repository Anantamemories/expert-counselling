export default function Home() {
  return (
    <div className="min-h-screen bg-gray-900 text-white font-sans">
      <header className="bg-black text-white p-6 shadow-md">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <h1 className="text-xl md:text-2xl font-bold">Expert Counselling 2025</h1>
          <a
            href="https://tagmango.app/b6973e66fb"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-yellow-400 text-black px-4 py-2 rounded-lg font-semibold hover:bg-yellow-300"
          >
            Enroll Now
          </a>
        </div>
      </header>

      <section className="bg-gradient-to-r from-gray-800 to-gray-900 py-16 px-4 text-center">
        <h2 className="text-2xl md:text-4xl font-bold mb-4">
          Get Into Top Engineering Colleges with Expert Help
        </h2>
        <p className="max-w-2xl mx-auto text-base md:text-lg mb-6 text-gray-300">
          Enroll in our JEE Mains 2025 Expert Counselling Program to access an on-demand course,
          personalized guidance, and choice filling support to get admission in IITs, NITs, IIITs, and top state colleges.
        </p>
        <a
          href="https://tagmango.app/b6973e66fb"
          target="_blank"
          rel="noopener noreferrer"
          className="bg-blue-600 hover:bg-blue-500 text-white font-semibold px-6 py-3 rounded-lg"
        >
          ENROLL NOW FOR EXPERT COUNSELLING 🚀
        </a>
      </section>

      <section className="py-16 px-4 max-w-5xl mx-auto">
        <h3 className="text-2xl md:text-3xl font-bold mb-8 text-center text-white">Why Join?</h3>
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3">
          {[
            'We cover almost all the reputed national & state level counselling ',
            'One-to-One Counselling Sessions',
            'We suggest the best possible colleges tailored to your preferences and potential.',
            'Support for JoSAA, CSAB, JAC, UPTAC & More',
            'Comprehensive Possibilities Report',
            'Money-Back Guarantee If No Admission',
            '24/7 Support Until Allotment'
          ].map((item, idx) => (
            <div
              key={idx}
              className="bg-gray-800 border border-gray-700 rounded-xl shadow p-6 hover:shadow-lg transition"
            >
              <p className="font-medium text-base md:text-lg text-gray-200">{item}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="bg-gray-800 py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-2xl md:text-3xl font-bold mb-6 text-center text-white">Success Stories</h3>
          <div className="space-y-8">
            {[
              {
                name: 'Jansi Bhati',
                college: 'IIT Bombay - Civil Engineering',
                quote:
                  'Thanks to Career Margdarshan, I got into IIT Bombay. Their team helped me choose the best options based on my rank.'
              },
              {
                name: 'Manan',
                college: 'NIT Trichy - Civil Engineering',
                quote:
                  'Their expert counselling helped me during the JoSAA process and made choice filling stress-free.'
              },
              {
                name: 'Shaswat Singh',
                college: 'IIIT Nagpur - CSE',
                quote:
                  'The entire counselling process was made simple with their guidance and personal attention.'
              }
            ].map((story, idx) => (
              <div key={idx} className="bg-gray-900 rounded-xl p-6 shadow">
                <p className="italic mb-2 text-sm md:text-base text-gray-300">"{story.quote}"</p>
                <p className="font-semibold text-sm md:text-base text-white">— {story.name}, {story.college}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="py-16 px-4 bg-gray-900">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-2xl md:text-3xl font-bold mb-8">Why Trust Us?</h3>
          <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3">
            {[
              { metric: '1000+', label: 'Students Guided' },
              { metric: '95%', label: 'Success Rate in Top Colleges' },
              { metric: '<10 min', label: 'Avg. Response Time' },
              { metric: '50+', label: 'Counsellors & Experts' },
              { metric: '7+ Years', label: 'Experience in Counselling' },
              { metric: '100%', label: 'Money-Back Assurance' }
            ].map((item, idx) => (
              <div
                key={idx}
                className="bg-gray-800 border border-gray-700 rounded-xl shadow p-6 hover:shadow-md transition"
              >
                <p className="text-3xl font-bold text-blue-400 mb-2">{item.metric}</p>
                <p className="font-medium text-sm md:text-base text-gray-300">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <footer className="bg-black text-white text-center py-6">
        <p>&copy; {new Date().getFullYear()} Expert Counselling by Career Mentors. All rights reserved.</p>
      </footer>
    </div>
  );
}
